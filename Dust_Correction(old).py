...
avydis = [] #average y distance
avydis_err = []

# a loop to run through each A_V value calculating the average y-distance between the intrinsic data above
# and the cluster data. The results from this loop are then used to find the best value of A_V as it will
# correspond to the minimum average distance.
for i in range(0, len(A_V)):
    ydistance = [] # list to store each iterations data to be averages
    dx = delx[i] # shift in x
    dy = dely[i] # shift in y
    for j in range(0, len(UBint)):

        # get shifted cluster data
        shifted_fake_cluster_x = fake_cluster_data[:,-1]-dx
        shifted_fake_cluster_y = fake_cluster_data[:,-2]-dy

        # check if it is close to a MS point within the tolerance
        if abs(BVint[j] - shifted_fake_cluster_x[j]+BVerr[j])<0.1:
            # find y-distance and add to list
            ydis = abs(UBint[j]-fake_cluster_data[:,-2][j])
            ydistance.append(ydis)
        else:
            continue
    avydis.append(np.mean(ydistance))
    avydis_err.append(np.std(ydistance))


min_index_y = np.argmin(avydis)
optimal_A_V = A_V[min_index_y] #find optimal value of A_V
...
ax['data'].errorbar(fake_cluster_data[:,-1], fake_cluster_data[:,-2],
             yerr=UBerr, xerr=BVerr, capsize=5, fmt='o', elinewidth=1, capthick=1,
...
ax['data'].errorbar(fake_cluster_data[:,-1]-delx, fake_cluster_data[:,-2]-dely,
             yerr=UBerr, xerr=BVerr, capsize=5, fmt='o', elinewidth=1, capthick=1,
             color='red', label=f'Dereddened Cluster Data, A_V={optimal_A_V}', markersize=1, zorder=2)
...
ax['A_V'].errorbar(A_V, avydis, yerr=avydis_err, xerr=None, capsize=5, fmt='o', elinewidth=1, capthick=1,)
ax['A_V'].set_xlabel('A_V values')
ax['A_V'].set_ylabel('average y-distance')
ax['A_V'].grid(True, axis='x')
ax['A_V'].tick_params(axis='x', which='major', labelsize=8)
ax['A_V'].set_xticks(A_V)
plt.show()